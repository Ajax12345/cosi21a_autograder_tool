...
public class GenericConsoleTester {
	 ...
	 * @see #getActual()
	 */
	public Queue<String> getActualFromByteStream() {
		...
		try {     
			...
			while (outRdr.hasNextLine()) {
				String trimmedNext = outRdr.nextLine().trim();
				if (!trimmedNext.isEmpty()) {
					actual.add(trimmedNext);
				}
			}
			outRdr.close();
			return actual;
	 ...
	 * @see #getActual()
	 */
	public Queue<String> getActualFromGeneratedFile() {
		...
		try {
			...
			while (actualRdr.hasNextLine()) {
				String trimmedNext = actualRdr.nextLine().trim();
				if (!trimmedNext.isEmpty()) {
					actual.add(trimmedNext);
				}
			}
